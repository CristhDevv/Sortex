...
export default function AssignmentsPage() {
  ...
  const groupedList = Object.values(
    assignments.reduce((acc, asg) => {
      const key = asg.vendor_id;
      if (!acc[key]) acc[key] = { vendor: { id: asg.vendor_id, ...asg.vendors }, items: [] };
      acc[key].items.push(asg);
      return acc;
    ...
    <div className="min-h-screen p-4 sm:p-6 space-y-6" style={{ background: 'var(--bg-page)' }}>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {groupedList.map((group: any, groupIdx: number) => (
            <div 
              key={groupIdx}
              className="rounded-2xl overflow-hidden shadow-2xl border"
              style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}
            >
              {/* Nivel 1: Header del Vendedor */}
              <div 
                className="p-4 border-b flex items-center gap-4 bg-indigo-500/5"
              ...
              {/* Nivel 2: Filas de Asignaciones */}
              <div className="divide-y divide-[var(--border)]">
                {group.items.map((asg: any) => {
                  ...
                  return (
                    <div 
                      key={asg.id} 
                      className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center transition-colors group gap-4"
                      onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--bg-card-hover)'; }}
                      onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; }}
                    >
                      <div className="flex flex-1 items-center justify-between sm:justify-start gap-4 sm:gap-6 w-full sm:w-auto ml-0 sm:ml-2">
                        {/* Lottery */}
                        ...
                        {/* Reports status */}
                        <div 
                          className="flex gap-1.5 items-center p-1.5 rounded-xl border ml-auto"
                        ...
                        {isEditing ? (
                          <div className="flex gap-1.5">
                            ...
                            >
                              {editingLoading ? <Loader2 size={18} className="animate-spin" /> : <Check size={18} />}
                            </button>
                            <button
                          ...
                          <div className="flex gap-1.5">
                            <button
                              ...
                              className={`p-2.5 rounded-xl border border-transparent transition-all ${isLiquidated ? 'opacity-30 cursor-not-allowed text-indigo-400' : 'text-indigo-400 hover:bg-indigo-500/10 hover:border-indigo-500/20'}`}
                            >
                              <Pencil size={18} />
                            </button>
                            <button
                ...
                {/* Botón + Lotería al final del grupo */}
                <button 
                  onClick={() => {
                    setSelectedVendor(group.vendor.id);
                    setIsVendorLocked(true);
                    setShowModal(true);
                  }}
                  className="w-full p-3 sm:p-4 flex items-center justify-center gap-2 text-sm font-bold border-t border-dashed transition-colors"
                  style={{ borderColor: 'var(--border)', color: 'var(--text-muted)' }}
                  onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--bg-card-hover)'; e.currentTarget.style.color = 'var(--text-primary)'; }}
                  onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'var(--text-muted)'; }}
                >
                  <Plus size={16} /> AGREGAR LOTERÍA
                </button>
              </div>
            </div>
          ))}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className={`w-full px-4 py-3.5 border font-medium rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all appearance-none ${isVendorLocked ? 'opacity-60 cursor-not-allowed' : ''}`}
                  style={{ background: 'var(--bg-card-hover)', borderColor: 'var(--border-hover)', color: 'var(--text-primary)' }}
                >
