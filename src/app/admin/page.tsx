...
export default function AdminDashboard() {
  ...
  return (
    <div 
      className="space-y-6 sm:space-y-8 p-4 sm:p-6 min-h-screen"
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <StatCard 
          ...
          textColor={isNearLimit ? "text-white" : "var(--text-primary)"}
          labelColor={isNearLimit ? "text-rose-100" : "var(--text-muted)"}
        />
      </div>
        ...
        <div className="bg-rose-500/10 border border-rose-500/30 p-6 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p 
                className="font-medium text-sm sm:text-base mt-1"
      ...
      <div 
        className="p-6 sm:p-8 rounded-2xl border shadow-2xl"
        style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}
      >
        <h2 
          className="text-xs sm:text-sm font-black uppercase tracking-widest mb-4"
          style={{ color: 'var(--text-primary)' }}
        >
          Estado del Sistema
        </h2>
...
function StatCard({ title, value, icon, bgColor, borderColor, textColor = "var(--text-primary)", labelColor = "var(--text-muted)" }: any) {
  return (
    <div 
      className="p-5 sm:p-6 rounded-2xl border flex items-center gap-4 transition-colors hover:border-[var(--border-hover)]"
      style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}
    >
      ...
      <div>
        <p 
          className={`text-[10px] sm:text-xs font-black uppercase tracking-widest mb-0.5 ${!labelColor.startsWith('var') ? labelColor : ''}`}
          style={labelColor.startsWith('var') ? { color: labelColor } : {}}
        >
          {title}
        </p>
        <p 
          className={`text-2xl sm:text-3xl font-black ${!textColor.startsWith('var') ? textColor : ''}`}
          style={textColor.startsWith('var') ? { color: textColor } : {}}
        >
          {value}
        </p>
      </div>
